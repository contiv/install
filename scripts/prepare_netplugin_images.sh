...
if [ -z "${NETPLUGIN_BRANCH:-}" ]; then
	...
	if [ "$http_rc" = 200 ]; then
		...
		echo "Failed to find contiv/netplugin:${CONTIV_NETPLUGIN_VERSION} on dockerhub, exit 1"
		exit 1
	fi
fi
...
if [ -z "${NETPLUGIN_AUTH_PROXY_BRANCH:-}" ]; then
	...
	# ensure the image exists
	http_rc=$(curl -s -w "%{http_code}" -o /dev/null https://hub.docker.com/v2/repositories/contiv/auth_proxy/tags/${auth_proxy_version}/)
	if [ "$http_rc" = 200 ]; then
		echo "Found contiv/auth_proxy:${auth_proxy_version} on dockerhub, exit 0"
		exit 0
	elif [ "$http_rc" = 404 ]; then
		echo "Failed to find contiv/auth_proxy:${auth_proxy_version} on dockerhub, exit 1"
		exit 1
	fi
fi
